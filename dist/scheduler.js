import { isWithinWorkingHours, getDelayUntilNextStart } from "./time.utils.js";
export function runScheduler(ctx) {
    if (ctx.state !== "running")
        return;
    ctx.checkAndResetDailyUsage();
    if (!isWithinWorkingHours(ctx.clock.now(), ctx.config.startTime, ctx.config.endTime)) {
        const delay = getDelayUntilNextStart(ctx.clock.now(), ctx.config.startTime, ctx.config.endTime);
    ...
    while (ctx.activeCalls < ctx.config.maxConcurrentCalls &&
